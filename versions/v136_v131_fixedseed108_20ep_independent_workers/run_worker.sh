...
[[ "${FIXED_SEED}" == "108" ]] || { echo "v136 requires FIXED_SEED=108" >&2; exit 2; }
[[ "${EPISODES_PER_WORKER}" == "4" ]] || { echo "v136 requires four attempts per worker" >&2; exit 2; }
...
for local_attempt in 0 1 2 3; do
  ...
  run_id="task24_v136_seed${FIXED_SEED}_w${WORKER_ID}_a${local_attempt}_$(date +%Y%m%d_%H%M%S)"
  ...
  PRIVATE_INPUTS_FILE="${PRIVATE_INPUTS_FILE}" \
    ...
    SEED="${FIXED_SEED}" \
    NUM_TRIALS=1 \
    STAMP="$(date +%Y%m%d_%H%M%S)_v136_w${WORKER_ID}_a${local_attempt}" \
