#!/usr/bin/env python3
"""Strictly aggregate the Task24 v132 five-node multiseed evaluation."""
...
def main() -> int:
    ...
    for worker_id in EXPECTED_WORKERS:
        ...
        for row in worker_rows:
            global_episode = int(row.get("global_episode", "-1"))
            expected_seed = SEED_START + global_episode
            if row.get("worker_id") != str(worker_id):
                errors.append(f"worker{worker_id}:bad_worker_id={row.get('worker_id')}")
            if row.get("seed") != str(expected_seed):
    ...
    summary = {
        "task_id": 24,
        "measurement": "multiseed_20ep",
        "seed_start": SEED_START,
        "seed_end": SEED_START + 19,
    ...
    (output_dir / "summary.tsv").write_text(
        "task_id\tmeasurement\tseed_start\tseed_end\trequested_episodes\tvalid_episodes\tstage_successes\tstage_success_rate\tgoal_successes\tgoal_success_rate\taverage_stage_score_pct\terrors\n"
        f"24\tmultiseed_20ep\t{SEED_START}\t{SEED_START + 19}\t20\t{len(valid_rows)}\t{stage_successes}\t"
