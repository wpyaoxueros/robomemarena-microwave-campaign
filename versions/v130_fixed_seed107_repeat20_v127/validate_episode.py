#!/usr/bin/env python3
"""Validate one independent Task21 V130 fixed-seed rollout."""
...
def main() -> int:
    ...
    snapshot_commit_path = run_dir / "exact_repro_snapshot" / "official_commit.txt"
    ...
    if not reasons:
        try:
            ...
            if not reasons:
                ...
                if summary.get("task_id") != "21" or summary.get("status") != "completed":
                    reasons.append("summary_not_completed_task21")
                if episode.get("task_id") != "21" or episode.get("ep") != "0":
                    reasons.append("episode_not_single_task21_ep0")
