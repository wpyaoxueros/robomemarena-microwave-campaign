...
def should_refresh_raw_vlm(
    effective_t: int,
    last_raw_vlm_t: int | None,
    interval: int,
) -> bool:
    """Keep raw-VLM diagnostics sparse without changing VLA action replanning."""
    if interval < 1:
        raise ValueError(f"interval must be positive, got {interval}")
    return last_raw_vlm_t is None or effective_t - last_raw_vlm_t >= interval
