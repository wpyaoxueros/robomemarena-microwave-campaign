...
RUN_ID="task22_v24_35999_v20_pickcookies_pregrasp160_robotonly_seed104_${STAMP}"
OUT_ROOT="${VERSION_DIR}/outputs/${RUN_ID}"
SESSION="task22v24_${STAMP}"
JOB_NAME="task22v24_${STAMP}"
...
tmux -f /dev/null -L hlei573borrow new-session -d -s "${SESSION}" \
  "bash -lc 'set -o pipefail; srun -p ${PARTITION} ${EXCLUDE_ARG} --gres=gpu:2 -c8 --mem=163840M --time=02:00:00 --job-name=${JOB_NAME} bash -lc \"cd ${VERSION_DIR} && ROBOMEMARENA_REMOTE_ROOT_OVERRIDE=${ROBOMEMARENA_REMOTE_ROOT_OVERRIDE} SLURM_EXCLUDE_NODES=${SLURM_EXCLUDE_NODES:-} PORT=19434 RUN_ID=${RUN_ID} OUT_ROOT=${OUT_ROOT} bash ${VERSION_DIR}/run_1ep.sh ${PRIVATE_INPUTS_FILE}\" 2>&1 | tee -a ${OUT_ROOT}/submit.log; rc=\\\${PIPESTATUS[0]}; echo [TMUX_EXIT] status=\\\${rc}; exec bash'"
