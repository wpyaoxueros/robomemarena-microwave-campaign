...
if metadata.get("object_anchor") is not False:
    raise SystemExit("v24 anchor must explicitly disable object_anchor")
if metadata.get("robot_only") is not True:
    raise SystemExit("v24 anchor must explicitly declare robot_only")
rules = payload.get("tasks", {}).get("22")
if not isinstance(rules, list) or len(rules) != 2:
    raise SystemExit("v24 requires exactly two Task22 initial-anchor rules")
...
for rule, (expected_subtask, expected_basename, expected_frame) in zip(rules, expected_rules, strict=True):
    if rule.get("subtask") != expected_subtask or int(rule.get("frame_idx", -1)) != expected_frame:
        raise SystemExit(f"v24 anchor must be frame {expected_frame} for {expected_subtask}")
    ...
    with h5py.File(hdf_path, "r") as handle:
        ...
        if expected_subtask == "pick cookies":
            ...
            if action_gripper >= 0.0:
                raise SystemExit("v24 pick-cookies anchor must remain before gripper closing")
...
print("TASK22_V24_ROBOT_ANCHORS_OK")
PY
