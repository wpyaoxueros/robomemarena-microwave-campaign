...
CPUS_PER_TASK=8
BATCH_ROOT="${OUTPUT_ROOT}/task21_v131_fixedseed107_fivenodes_repeat20_${STAMP}"
...
for candidate in "${PARTITIONS[@]}"; do
  if srun --immediate=20 -p "${candidate}" --gres=gpu:1 -c1 --mem=1024M --time=00:01:00 \
    --job-name="task21v131probe_${STAMP}" bash -lc 'nvidia-smi --query-gpu=name --format=csv,noheader | head -1 >/dev/null' </dev/null; then
    PARTITION="${candidate}"
    break
  fi
done
...
SESSION="task21_v131_${STAMP}_five_nodes"
JOB_NAME="task21v131_${STAMP}_five_nodes"
...
INNER="set -o pipefail; srun -p ${PARTITION} --nodes=5 --ntasks=5 --ntasks-per-node=1 --gres=gpu:2 -c${CPUS_PER_TASK} --mem=${MEM_MB}M --time=02:00:00 --job-name=${JOB_NAME} --output=${TASK_LOG} bash ${RUNNER_Q} ${ENV_DIR_Q} 2>&1 | tee -a ${LOG_Q}; rc=\${PIPESTATUS[0]}; echo \"[TMUX_EXIT] status=\${rc}\"; exit \${rc}"
tmux -f /dev/null -L hlei573borrow new-session -d -s "${SESSION}" \
  "bash -lc $(printf '%q' "${INNER}")"

printf 'status=submitted\nsession=%s\njob_name=%s\npartition=%s\nrequested_nodes=5\nfinished_at=%s\n' \
  "${SESSION}" "${JOB_NAME}" "${PARTITION}" "$(date -Is)" >>"${BATCH_ROOT}/LIVE_STATUS.txt"
printf '%s\n' "${BATCH_ROOT}"
