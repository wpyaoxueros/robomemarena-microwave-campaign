...
run_probe() {
  ...
  srun -p acd_u --gres=gpu:2 -c16 --mem="${MEM_MB}M" --time=00:15:00 --job-name="task22v3bs${batch}_${STAMP}" \
    bash -lc "cd ${VERSION_DIR} && PRIVATE_INPUTS_FILE=${PRIVATE_INPUTS_FILE} VLM_INPUT_CHECKPOINT=${COMPAT_MODEL_DIR} TASK22_V3_DATASET=${TASK22_V3_DATASET} TASK22_V3_DATA_MANIFEST=${TASK22_V3_DATA_MANIFEST} PROBE_OUTPUT_ROOT=${DISPATCH_ROOT}/probes PROBE_BATCH=${batch} MASTER_PORT=${port} STAMP=${STAMP}_bs${batch} bash ${VERSION_DIR}/probe_one_batch.sh" \
    2>&1 | tee -a "${log}"
...
srun -p acd_u --gres=gpu:2 -c16 --mem="${MEM_MB}M" --time=02:00:00 --job-name="task22v3train_${STAMP}" \
  bash -lc "cd ${VERSION_DIR} && PRIVATE_INPUTS_FILE=${PRIVATE_INPUTS_FILE} VLM_INPUT_CHECKPOINT=${COMPAT_MODEL_DIR} TASK22_V3_DATASET=${TASK22_V3_DATASET} TASK22_V3_DATA_MANIFEST=${TASK22_V3_DATA_MANIFEST} TRAIN_OUTPUT_ROOT=${TRAIN_OUTPUT_ROOT} RUN_ID=${RUN_ID} PER_DEVICE_BS=${SELECTED_BS} MASTER_PORT=29626 STAMP=${STAMP}_train bash ${VERSION_DIR}/run_train.sh" \
