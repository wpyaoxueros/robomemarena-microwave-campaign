...
: "${ROBOMEMARENA_REMOTE_ROOT:?set ROBOMEMARENA_REMOTE_ROOT before submitting}"
...
for partition in acd_u acd_ue emergency_acd; do
  if srun --immediate=20 -p "${partition}" --gres=gpu:1 -c1 --mem=1024M --time=00:01:00 \
    --job-name="task22v12_1gpu_${STAMP}" bash -lc 'whoami; hostname; nvidia-smi -L' \
    >"${PROBE_DIR}/${partition}_1gpu.log" 2>&1; then
    if srun --immediate=20 -p "${partition}" --gres=gpu:2 -c8 --mem=163840M --time=00:01:00 \
      --job-name="task22v12_2gpu_${STAMP}" bash -lc 'whoami; hostname; nvidia-smi -L' \
...
PARTITION="${selected_partition}" \
  ROBOMEMARENA_REMOTE_ROOT="${ROBOMEMARENA_REMOTE_ROOT}" \
