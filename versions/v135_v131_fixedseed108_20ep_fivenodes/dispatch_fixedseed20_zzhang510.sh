...
BATCH_ROOT="${OUTPUT_ROOT}/task24_v135_v131_fixedseed108_20ep_fivenodes_${STAMP}"
RUNTIME_ENV_DIR="${BATCH_ROOT}/runtime_env"
mkdir -p "${RUNTIME_ENV_DIR}"
[[ -z "$(git -C "${REPO_DIR}" status --porcelain -- "${REL_VERSION}" 2>/dev/null)" ]] || {
  echo "refuse to launch from a dirty v135 version directory" >&2
  exit 2
}
...
SESSION="task24_v135_${STAMP}_five_nodes"
JOB_NAME="task24v135_${STAMP}_five_nodes"
...
printf 'status=submitted\nsession=%s\njob_name=%s\npartition=%s\nrequested_nodes=%s\nsubmitted_at=%s\n' \
  "${SESSION}" "${JOB_NAME}" "${SLURM_PARTITION}" "${WORKERS}" "$(date -Is)" >>"${BATCH_ROOT}/LIVE_STATUS.txt"
printf '%s\n' "${BATCH_ROOT}"
