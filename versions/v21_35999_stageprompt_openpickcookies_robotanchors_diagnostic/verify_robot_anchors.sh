...
if metadata.get("object_anchor") is not False:
    raise SystemExit("v21 anchor must explicitly disable object_anchor")
if metadata.get("robot_only") is not True:
    raise SystemExit("v21 anchor must explicitly declare robot_only")
rules = payload.get("tasks", {}).get("22")
if not isinstance(rules, list) or len(rules) != 2:
    raise SystemExit("v21 requires exactly two Task22 initial-anchor rules")
...
for rule, (expected_subtask, expected_basename) in zip(rules, expected_rules, strict=True):
    if rule.get("subtask") != expected_subtask or int(rule.get("frame_idx", -1)) != 0:
        raise SystemExit(f"v21 anchor must be frame 0 for {expected_subtask}")
...
print("TASK22_V21_ROBOT_ANCHORS_OK")
PY
