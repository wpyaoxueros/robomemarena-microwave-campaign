...
PACK_DIR="${TASK22_V19_PACK_DIR_OVERRIDE:?set TASK22_V19_PACK_DIR_OVERRIDE to the frozen package root}"
EVAL_PY=${EVAL_PY_OVERRIDE:-${PACK_DIR}/evaluators/eval_tasks2_26_sync_endpose_hold_officialscore.py}

if [[ "${TASK22_V19_WIRING_ONLY:-0}" == "1" ]]; then
  printf 'V19_ACTIVE_EVAL_PY=%s\n' "${EVAL_PY}"
  exit 0
fi
