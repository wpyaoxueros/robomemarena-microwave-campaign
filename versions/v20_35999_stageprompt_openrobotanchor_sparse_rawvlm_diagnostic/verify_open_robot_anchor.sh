...
if metadata.get("object_anchor") is not False:
    raise SystemExit("v20 anchor must explicitly disable object_anchor")
if metadata.get("robot_only") is not True:
    raise SystemExit("v20 anchor must explicitly declare robot_only")
rules = payload.get("tasks", {}).get("22")
if not isinstance(rules, list) or len(rules) != 1:
    raise SystemExit("v20 requires exactly one Task22 initial-anchor rule")
rule = rules[0]
if rule.get("subtask") != "open microwave" or int(rule.get("frame_idx", -1)) != 0:
    raise SystemExit("v20 anchor must be frame 0 for open microwave")
...
print("TASK22_V20_OPEN_ROBOT_ANCHOR_OK")
PY
