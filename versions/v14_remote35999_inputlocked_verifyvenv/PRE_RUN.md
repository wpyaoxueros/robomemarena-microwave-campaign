# Task22 v14 Original-35999 Input-Locked Baseline

## Purpose

Evaluate Task22 with the repaired remote stage contract using the original
`35999` VLA and its matching norm, not the legacy `2999` runtime. The repaired
remote scorer restores the Task22 sequence after the two tomato-sauce pours:
...
but is excluded from stage success and stage score. This baseline has no
oracle prompt injection, object anchor, completed-subtask injection, or
forward-stage lock; VLM supplies the prompts and the only retained guard is
