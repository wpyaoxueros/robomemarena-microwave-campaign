...
VLA_NORM_ASSET_ID=$(basename "${VLA_REPO_ID}")
VLA_NORM_FILE="${VLA_REPO_ID}/norm_stats.json"
CHECKPOINT_NORM_FILE="${VLA_POLICY}/assets/${VLA_NORM_ASSET_ID}/norm_stats.json"
[[ -r "${VLA_NORM_FILE}" ]] || { echo "missing runtime norm stats: ${VLA_NORM_FILE}" >&2; exit 4; }
[[ -r "${CHECKPOINT_NORM_FILE}" ]] || { echo "missing checkpoint norm asset: ${CHECKPOINT_NORM_FILE}" >&2; exit 4; }
cmp -s "${VLA_NORM_FILE}" "${CHECKPOINT_NORM_FILE}" || {
  echo "runtime and checkpoint norm stats differ" >&2
  exit 4
}
...
OUTPUT_ROOT=${OUTPUT_ROOT:-${OUT_BASE:?OUT_BASE is required when OUT_ROOT is unset}}
...
if [[ -n "${VLA_TRAINING_PROMPT_TEMPLATE_FILE}" ]]; then
  cp "${VLA_TRAINING_PROMPT_TEMPLATE_FILE}" "${OUT_ROOT}/code_snapshot/vla_training_prompt_template.json"
fi
