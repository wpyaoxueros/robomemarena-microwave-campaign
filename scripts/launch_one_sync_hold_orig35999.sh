...
if [[ "${TASK_ID}" != 21 ]]; then
  echo "this frozen package only supports TASK_ID=21" >&2
  exit 2
fi

PACK_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
EXPECTED_OFFICIAL_COMMIT=62214036103ee8d5fef9b475dd8b344b6e2cfc03
REMOTE_ROOT=${ROBOMEMARENA_REMOTE_ROOT:?set ROBOMEMARENA_REMOTE_ROOT to the fixed official checkout}
...
OPENPI_ROOT=${OPENPI_ROOT:?set OPENPI_ROOT}
INFER_ROOT=${INFER_ROOT:?set INFER_ROOT}
TARGET_LIBERO_PATH=${TARGET_LIBERO_PATH:?set TARGET_LIBERO_PATH}

VLA_POLICY=${VLA_POLICY:?set VLA_POLICY}
VLA_CONFIG=${VLA_CONFIG:-pi05_libero_robomemarena_fullvlm_v2_noflip_dataset}
VLA_REPO_ID=${VLA_REPO_ID:-${PACK_DIR}/assets/norm_repo}
...
OUTPUT_ROOT=${OUTPUT_ROOT:-${PACK_DIR}/outputs}
