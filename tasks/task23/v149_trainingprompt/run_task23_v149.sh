...
for required in OPENPI_ROOT INFER_ROOT TARGET_LIBERO_PATH ROBOMEMARENA_REMOTE_ROOT VLA_POLICY VLM_CKPT; do
...
export VLA_REPO_ID="${VLA_REPO_ID:-${ROOT}/runtime_assets/robomemarena_fullvlm_v2_noflip_dataset_v2}"
[[ -r "${VLA_REPO_ID}/norm_stats.json" ]] || {
  echo "missing package norm stats: ${VLA_REPO_ID}/norm_stats.json" >&2
  exit 2
}
export VLA_TRAINING_PROMPT_TEMPLATE_FILE="${VLA_TRAINING_PROMPT_TEMPLATE_FILE:-${ROOT}/config/vla_training_prompt_task23_from35999.json}"
[[ -s "${VLA_TRAINING_PROMPT_TEMPLATE_FILE}" ]] || {
  echo "missing VLA training prompt template: ${VLA_TRAINING_PROMPT_TEMPLATE_FILE}" >&2
  exit 2
}
export RUN_ID="${RUN_ID:-task23_v149_trainingprompt}"
...
export ENDPOSE_HOLD_CONSECUTIVE_BY_SUBTASK_JSON='{"open microwave":1,"place cream":3,"place cookies":3,"place popcorn":1}'
