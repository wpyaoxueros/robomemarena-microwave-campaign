# RoboMemArena Task23 v148 Reproduction

这是 Task23 的一个不可变评测版本仓库。每个版本一个 GitHub 仓库；不覆盖之前版本。

## 版本结论

- 继承版本：Task23 v147，父版本冻结 commit 为 `b342c5c`。
- 唯一修改：过期的快照 helper 改为“文件存在时才复制”；该文件不参与 rollout，避免在
  VLA/VLM 加载前因 snapshot 失败退出。
- EEF hold 和自包含 norm 与 v147 完全相同：`place popcorn` 连续命中仍为 `1`，启动仍会
  强制检查 runtime norm 与 checkpoint asset 逐字节一致。
...
发现新假设时，创建一个新的 GitHub 仓库和新的版本号；不要在本仓库修改配置后冒充 v148。
该规则避免历史成功代码和后续尝试混淆。
