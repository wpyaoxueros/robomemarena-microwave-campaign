# RoboMemArena Task23 v147 Reproduction

这是 Task23 的一个不可变评测版本仓库。每个版本一个 GitHub 仓库；不覆盖之前版本。

## 版本结论

- 继承版本：Task23 v146，父版本冻结 commit 为 `a7f7a8a`。
- 唯一修改：将 original fullvlmv2 的精确 norm 作为包内 runtime asset，并安装到
  同一 VLA checkpoint 的 `assets/<asset_id>/norm_stats.json`；启动时强制检查两个文件
  内容完全相同，禁止隐式 fallback。
- EEF hold 行为与 v146 完全相同：`place popcorn` 连续命中要求仍为 `1`。
- VLM 负责输出所有子任务 prompt；没有 oracle next-prompt 注入。
- 没有 object anchor 或物体瞬移。
- 仍保留 v145 的三个 robot-only release anchor；它们只会在 VLM 已输出下一 prompt 且当前子任务完成 EEF hold/release 后执行。
