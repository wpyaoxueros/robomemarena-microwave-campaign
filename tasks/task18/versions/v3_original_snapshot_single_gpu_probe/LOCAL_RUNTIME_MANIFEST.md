# Task18 v3 single-GPU local runtime manifest

This manifest records the exact local assets used for the single-GPU
historical-path probe. These paths are intentionally versioned for
reproducibility.
...
```text
task=18
num_trials=1 (probe default; can be explicitly raised after a successful probe)
seed=104
max_steps=2200
replan_steps=5
...
This version changes only execution topology: VLA and VLM share one allocated
GPU. It must not be described as the historical two-GPU runtime reproduction.
