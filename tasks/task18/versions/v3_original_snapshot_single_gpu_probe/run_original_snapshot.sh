...
export VLM_CUDA_VISIBLE_DEVICES="${VLM_CUDA_VISIBLE_DEVICES:-0}"
if [[ "${VLA_CUDA_VISIBLE_DEVICES}" != "${VLM_CUDA_VISIBLE_DEVICES}" ]]; then
  echo "[ERROR] v3 requires VLA and VLM to share one visible GPU" >&2
  exit 4
fi
