...
CPUS_PER_TASK=8
BATCH_ROOT="${OUTPUT_ROOT}/task21_v133_fixedseed107_pinnedfastnodes_repeat20_${STAMP}"
...
printf 'status=started\nstarted_at=%s\nfixed_seed=%s\nworkers=5\nrepeats_per_worker=%s\nrequested_nodes=5\nfast_nodes=%s\n' \
  "$(date -Is)" "${FIXED_SEED}" "${REPEATS}" "${FAST_NODES}" >"${BATCH_ROOT}/LIVE_STATUS.txt"

PARTITION=""
for candidate in "${PARTITIONS[@]}"; do
  if srun --immediate=20 -p "${candidate}" --nodelist="${FAST_NODES}" --gres=gpu:1 -c1 --mem=1024M --time=00:01:00 \
    --job-name="task21v133probe_${STAMP}" bash -lc 'nvidia-smi --query-gpu=name --format=csv,noheader | head -1 >/dev/null' </dev/null; then
    PARTITION="${candidate}"
    break
  fi
done
[[ -n "${PARTITION}" ]] || { echo "no verified-fast-node GPU probe succeeded" >&2; exit 3; }
...
SESSION="task21_v133_${STAMP}_pinned_fast_nodes"
JOB_NAME="task21v133_${STAMP}_pinned_fast_nodes"
