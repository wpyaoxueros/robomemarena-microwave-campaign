...
def run_episode_sync_endpose_hold(
    ...
    **_: Any,
    ...
    task22_stageprompt_diagnostic = env_bool("TASK22_STAGEPROMPT_DIAGNOSTIC", False)
    if task22_stageprompt_diagnostic and task_id_int != 22:
        raise ValueError("TASK22_STAGEPROMPT_DIAGNOSTIC can only run Task22")
    task22_pick_cookies_target = None
    task22_pick_cookies_tol = 0.08
    task22_pick_cookies_consecutive = 2
    task22_raw_vlm_interval = 1
    if task22_stageprompt_diagnostic:
        raw_target_path = os.environ.get("TASK22_STAGEPROMPT_PICK_COOKIES_TARGET_JSON", "").strip()
        if not raw_target_path:
            raise ValueError(
                "TASK22_STAGEPROMPT_DIAGNOSTIC requires "
                "TASK22_STAGEPROMPT_PICK_COOKIES_TARGET_JSON"
            )
        task22_pick_cookies_target = load_pick_cookies_target(Path(raw_target_path))
        task22_pick_cookies_tol = env_float("TASK22_STAGEPROMPT_PICK_COOKIES_TOL", 0.08)
        task22_pick_cookies_consecutive = max(
            1,
            env_int("TASK22_STAGEPROMPT_PICK_COOKIES_CONSECUTIVE", 2),
        )
        task22_raw_vlm_interval = max(
            1,
            env_int("TASK22_STAGEPROMPT_RAW_VLM_INTERVAL", 1),
        )
        logger.info(
            "[TASK22_STAGEPROMPT_RAW_VLM_INTERVAL] interval=%s; VLA replan_steps=%s is unchanged",
            task22_raw_vlm_interval,
            args.replan_steps,
        )
    ...
    task22_pick_cookies_ready = False
    task22_pick_cookies_streak = 0
    last_raw_vlm_subtask = ""
    last_raw_vlm_t: int | None = None
    ...
    def save_vlm_debug_frame(raw_subtask: str, normalized_subtask: str) -> None:
        ...
        try:
            ...
            element = base.obs_to_pi_element(obs, resize_size=args.resize_size, prompt=prompt)
    ...
    def update_stage_and_goal(done: bool) -> bool:
        ...
        nonlocal task22_pick_cookies_ready, task22_pick_cookies_streak
        ...
        if (
            task22_stageprompt_diagnostic
            and official_stage_idx == 5
            and not task22_pick_cookies_ready
            and task22_pick_cookies_target is not None
        ):
            task22_pick_dist = float(
                np.linalg.norm(get_eef_pos(obs) - task22_pick_cookies_target.position)
            )
            if task22_pick_dist <= task22_pick_cookies_tol:
                task22_pick_cookies_streak += 1
            else:
                task22_pick_cookies_streak = 0
            if task22_pick_cookies_streak >= task22_pick_cookies_consecutive:
                task22_pick_cookies_ready = True
                logger.info(
                    "[TASK22_STAGEPROMPT_PICK_COOKIES_READY] t=%s dist=%.5f tol=%.5f streak=%s",
                    t,
                    task22_pick_dist,
                    task22_pick_cookies_tol,
                    task22_pick_cookies_streak,
                )
    ...
    def maybe_update_endpose_streak(subtask: str, phase: str, t_now: int) -> bool:
        ...
        if pick_height_applies:
            ...
            direction_gate_applies = subtask in direction_signatures
            passage_ok = (
                True
                if direction_gate_applies
                else target_passage_count.get(subtask, 0) >= required_passages
            )
            direction_ok, direction_cos, direction_disp, prev_dist, direction_reason = (
    ...
    try:
        ...
        while t < args.max_steps + args.num_steps_wait:
            ...
            elif task22_stageprompt_diagnostic and not should_refresh_raw_vlm(
                effective_t,
                last_raw_vlm_t,
                task22_raw_vlm_interval,
            ):
                latest_subtask = last_raw_vlm_subtask
                logger.info(
                    "[TASK22_STAGEPROMPT_RAW_VLM_CACHED] t=%s task=%s source_t=%s interval=%s raw_subtask=%s",
                    t,
                    planner.task_info.task_id,
                    last_raw_vlm_t,
                    task22_raw_vlm_interval,
                    latest_subtask or "<none>",
                )
            else:
                latest_subtask = planner.infer_sync(effective_t, clone_recent_frames())
                if task22_stageprompt_diagnostic:
                    last_raw_vlm_subtask = str(latest_subtask)
                    last_raw_vlm_t = effective_t
            ...
            if task22_stageprompt_diagnostic:
                scheduled_subtask = prompt_for_stage(
                    official_stage_idx,
                    task22_pick_cookies_ready,
                )
                if latest_subtask != scheduled_subtask:
                    logger.info(
                        "[TASK22_STAGEPROMPT_DIAGNOSTIC] t=%s official_stage_idx=%s "
                        "raw_subtask=%s scheduled_subtask=%s cookies_pick_ready=%s",
                        t,
                        official_stage_idx,
                        latest_subtask or "<none>",
                        scheduled_subtask,
                        int(task22_pick_cookies_ready),
                    )
                latest_subtask = scheduled_subtask
