...
GPU_GATE_DIR="${REPO_DIR}/versions/v4_device_binding_preflight_20260722"
...
for required_file in "${PRIVATE_INPUTS_FILE}" "${TASK22_V3_DATASET}" "${TASK22_V3_DATA_MANIFEST}" \
  "${GPU_GATE_DIR}/collect_gpu_preflight.py" "${GPU_GATE_DIR}/probe_gpu_binding.py"; do
...
RUN_ID="${RUN_ID:-task22_v5_native_boundary_${STAMP}}"
...
GPU_GATE_PORT="${GPU_GATE_PORT:-29641}"
...
fi
{
  ...
  printf 'gpu_gate_version=v4_device_binding_preflight_20260722\n'
...
"${OPENPI_PYTHON}" "${GPU_GATE_DIR}/collect_gpu_preflight.py" \
  --report "${RUN_ROOT}/gpu_preflight.json" --expected-world-size 2 --require-clean 2>&1 | tee -a "${LOG_FILE}"
"${OPENPI_PYTHON}" -m torch.distributed.run --nproc_per_node=2 --master_port="${GPU_GATE_PORT}" \
  "${GPU_GATE_DIR}/probe_gpu_binding.py" --report "${RUN_ROOT}/rank_binding.json" 2>&1 | tee -a "${LOG_FILE}"
