...
PACK_DIR="${TASK22_V21_PACK_DIR_OVERRIDE:?set TASK22_V21_PACK_DIR_OVERRIDE to the frozen package root}"
EVAL_PY=${EVAL_PY_OVERRIDE:-${PACK_DIR}/evaluators/eval_tasks2_26_sync_endpose_hold_officialscore.py}

if [[ "${TASK22_V21_WIRING_ONLY:-0}" == "1" ]]; then
  printf 'V21_ACTIVE_EVAL_PY=%s\n' "${EVAL_PY}"
  exit 0
fi
...
if ROBOMEMARENA_OFFICIAL_COMMIT=$(git -C "${REMOTE_ROOT}" rev-parse HEAD 2>/dev/null); then
  :
...
if [[ "${ROBOMEMARENA_OFFICIAL_COMMIT}" != "${EXPECTED_OFFICIAL_COMMIT}" ]]; then
 ...
 : "${VLA_CONFIG:?set VLA_CONFIG}"
 : "${VLA_REPO_ID:?set VLA_REPO_ID}"
VLA_SERVER_PY=${VLA_SERVER_PY:-${PACK_DIR}/scripts/serve_policy_custom_repo.py}

case "${TASK_ID}" in
  20)
    ENDPOSE_HOLD_POS_TOL=${ENDPOSE_HOLD_POS_TOL:-0.04}
    ENDPOSE_HOLD_EEF_DEFAULT_TOL=${ENDPOSE_HOLD_EEF_DEFAULT_TOL:-0.04}
    ENDPOSE_HOLD_EEF_P95_EXTRA_TOL=${ENDPOSE_HOLD_EEF_P95_EXTRA_TOL:-0.00}
    ENDPOSE_HOLD_EEF_TOL_CAP=${ENDPOSE_HOLD_EEF_TOL_CAP:-0.04}
    ENDPOSE_PICK_GRIPPER_GATE=${ENDPOSE_PICK_GRIPPER_GATE:-0}
    ENDPOSE_PICK_OBJECT_LIFT_GATE=${ENDPOSE_PICK_OBJECT_LIFT_GATE:-0}
    ;;
  21)
...
REQUIRE_HOLD_RELEASE_FOR_FORWARD=${REQUIRE_HOLD_RELEASE_FOR_FORWARD:-0}
REQUIRE_HOLD_RELEASE_FOR_FORWARD_SUBTASKS=${REQUIRE_HOLD_RELEASE_FOR_FORWARD_SUBTASKS:-}
...
mkdir -p \
  ...
  "${OUT_ROOT}/code_snapshot/runtime_support"
...
cp "${EVAL_PY}" "${OUT_ROOT}/code_snapshot/active_evaluator.py"
if [[ -f "$(dirname "${EVAL_PY}")/task22_stageprompt.py" ]]; then
  cp "$(dirname "${EVAL_PY}")/task22_stageprompt.py" "${OUT_ROOT}/code_snapshot/runtime_support/"
fi
if [[ -n "${INITIAL_SUBTASK_ANCHORS_JSON}" ]]; then
  cp "${INITIAL_SUBTASK_ANCHORS_JSON}" "${OUT_ROOT}/code_snapshot/initial_subtask_anchors.json"
fi
cp "${PACK_DIR}/config/"*.json "${OUT_ROOT}/code_snapshot/"
for optional_script in build_microwave_deep_eef_targets.py build_microwave_pick_contact_eef_targets.py; do
  if [[ -f "${PACK_DIR}/scripts/${optional_script}" ]]; then
    cp "${PACK_DIR}/scripts/${optional_script}" "${OUT_ROOT}/code_snapshot/"
  fi
done
...
cat > "${OUT_ROOT}/run_manifest.json" <<JSON
{
  ...
  "require_hold_release_for_pick_forward_subtasks": "${REQUIRE_HOLD_RELEASE_FOR_PICK_FORWARD_SUBTASKS}",
  "require_hold_release_for_forward": ${REQUIRE_HOLD_RELEASE_FOR_FORWARD},
  "require_hold_release_for_forward_subtasks": "${REQUIRE_HOLD_RELEASE_FOR_FORWARD_SUBTASKS}",
  ...
  "initial_subtask_anchors_json": "${INITIAL_SUBTASK_ANCHORS_JSON}",
  "task22_stageprompt_raw_vlm_interval": ${TASK22_STAGEPROMPT_RAW_VLM_INTERVAL:-1},
...
export REQUIRE_HOLD_RELEASE_FOR_FORWARD REQUIRE_HOLD_RELEASE_FOR_FORWARD_SUBTASKS
...
export ENDPOSE_PLACE_RELEASE_EEF_GUARD ENDPOSE_PLACE_RELEASE_EEF_GUARD_GRIPPER_VALUE
