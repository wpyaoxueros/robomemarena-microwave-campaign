...
export RUN_ID="${RUN_ID:-task22_v8_legacy_seed104_$(date +%Y%m%d_%H%M%S)}"
...
export SERVER_LOG="${LOG_DIR}/serve_policy_original_runtime.log"
export EVAL_LOG="${LOG_DIR}/eval_original_runtime.log"
...
printf '%s\n' \
  ...
  'replan_steps=10' \
  'async_vlm=1' \
  'oracle_prompt_injection=absent_from_runtime' \
  "node=$(hostname)" \
...
[[ "${READY}" == 1 ]] || { echo 'legacy VLA server did not become ready' >&2; exit 1; }
