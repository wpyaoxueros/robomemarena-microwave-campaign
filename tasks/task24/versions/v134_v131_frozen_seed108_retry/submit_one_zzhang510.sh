...
SLURM_PARTITION="${SLURM_PARTITION:-acd_u}"
...
tmux -f /dev/null -L hlei573borrow new-session -d -s "${SESSION}" \
  "bash -lc 'set -o pipefail; srun -p ${SLURM_PARTITION} --gres=gpu:2 -c${CPUS_PER_TASK} --mem=${MEM_MB}M --time=02:00:00 --job-name=${JOB_NAME} bash -lc \"cd ${VERSION_DIR} && CUDA_VISIBLE_DEVICES=0,1 PRIVATE_INPUTS_FILE=${PRIVATE_INPUTS_FILE} SEED=${SEED} PORT=${PORT} OUTPUT_ROOT=${OUTPUT_ROOT} RUN_ID=${RUN_ID} OUT_ROOT=${OUT_ROOT} bash ${VERSION_DIR}/run_one.sh\" 2>&1 | tee -a ${OUT_ROOT}/submit.log; rc=\\\${PIPESTATUS[0]}; echo [TMUX_EXIT] status=\\\${rc}; exec bash'"
printf 'session=%s\njob_name=%s\nout_root=%s\nseed=%s\npartition=%s\n' "${SESSION}" "${JOB_NAME}" "${OUT_ROOT}" "${SEED}" "${SLURM_PARTITION}"
