...
REMOTE_ROOT=${ROBOMEMARENA_REMOTE_ROOT:?set ROBOMEMARENA_REMOTE_ROOT to the frozen RoboMemArena checkout at ${EXPECTED_OFFICIAL_COMMIT}}
...
OPENPI_ROOT=${OPENPI_ROOT:?set OPENPI_ROOT in private inputs.env}
INFER_ROOT=${INFER_ROOT:?set INFER_ROOT in private inputs.env}
TARGET_LIBERO_PATH=${TARGET_LIBERO_PATH:?set TARGET_LIBERO_PATH in private inputs.env}

VLA_POLICY=${VLA_POLICY:?set VLA_POLICY in private inputs.env}
VLA_CONFIG=${VLA_CONFIG:?set VLA_CONFIG in private inputs.env}
VLA_REPO_ID=${VLA_REPO_ID:?set VLA_REPO_ID in private inputs.env}
VLA_SERVER_PY=${VLA_SERVER_PY:-${PACK_DIR}/scripts/serve_policy_custom_repo.py}
VLM_CKPT=${VLM_CKPT:?set VLM_CKPT in private inputs.env}
...
OUTPUT_ROOT=${OUTPUT_ROOT:?set OUTPUT_ROOT in private inputs.env}
