...
# Two-GPU counterpart to the archived direct-use launcher.  It deliberately
# retains each task's recorded VLM/hold inputs and uses the existing d9
# callback adapter; only the VLA/VLM process placement changes from the known
# invalid single-GPU topology.
TASK_ID=${1:?usage: run_archived_direct20_two_gpu.sh TASK_ID}
...
case "${TASK_ID}" in
  ...
  26)
    ...
    ;;
  *)
    echo "unsupported direct archived two-GPU task: ${TASK_ID}" >&2
    exit 2
    ;;
esac
...
[[ ${#visible_gpus[@]} -eq 2 ]] || {
  echo "direct two-GPU run requires exactly two visible GPUs, got ${CUDA_VISIBLE_DEVICES}" >&2
  exit 2
}
...
RUN_ID=${RUN_ID:-task${TASK_ID}_all18_direct20_two_gpu_seed104_${STAMP}}
cat >"${OUTPUT_ROOT}/launch_records/${RUN_ID}.env" <<EOF
task_id=${TASK_ID}
run_id=${RUN_ID}
kind=all18_direct_archived_two_gpu
...
export VLM_CUDA_VISIBLE_DEVICES=1
