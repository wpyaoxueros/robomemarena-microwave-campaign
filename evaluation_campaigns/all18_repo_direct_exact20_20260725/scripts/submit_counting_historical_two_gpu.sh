...
TASK_ID=${1:?usage: submit_counting_historical_two_gpu.sh TASK_ID}
CAMPAIGN_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
REPO_DIR="$(cd "${CAMPAIGN_DIR}/../.." && pwd)"
RUNNER="${CAMPAIGN_DIR}/scripts/run_counting_historical_two_gpu.sh"
...
SESSION=${SESSION:-lhs_t${TASK_ID}_historical_d9_${STAMP}}
JOB_NAME=${JOB_NAME:-lhs_t${TASK_ID}_historical_d9_${STAMP}}
