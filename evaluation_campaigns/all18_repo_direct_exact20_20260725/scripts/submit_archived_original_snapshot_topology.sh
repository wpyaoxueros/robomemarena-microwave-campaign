...
TASK_ID=${1:?usage: submit_archived_original_snapshot_topology.sh TASK_ID}
CAMPAIGN_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
REPO_DIR="$(cd "${CAMPAIGN_DIR}/../.." && pwd)"
RUNNER="${CAMPAIGN_DIR}/scripts/run_archived_original_snapshot_topology.sh"
...
SESSION=${SESSION:-lhs_t${TASK_ID}_originalsnapshot66e789_${STAMP}}
JOB_NAME=${JOB_NAME:-lhs_t${TASK_ID}_originalsnapshot66e789_${STAMP}}
