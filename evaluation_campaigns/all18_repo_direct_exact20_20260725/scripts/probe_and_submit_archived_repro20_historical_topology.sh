...
# Run the mandatory account-local probes and submit the exact two-GPU replay
# from the same shell. Probe status is written to stderr so command substitution
# receives only the selected partition.

TASK_ID=${1:?usage: probe_and_submit_archived_repro20_historical_topology.sh TASK_ID}
CAMPAIGN_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SUBMITTER="${CAMPAIGN_DIR}/scripts/submit_archived_repro20_historical_topology.sh"
OUTPUT_ROOT=${OUTPUT_ROOT:?set OUTPUT_ROOT to the submit-account output root}
STAMP=${STAMP:-$(date +%Y%m%d_%H%M%S)}
PROBE_LOG_DIR="${OUTPUT_ROOT}/probe_logs"
PROBE_LOG="${PROBE_LOG_DIR}/task${TASK_ID}_historical66e789_${STAMP}.log"
...
OUTPUT_ROOT="${OUTPUT_ROOT}" PARTITION="${TWO_GPU_PARTITION}" STAMP="${STAMP}" \
  RUN_ID="task${TASK_ID}_historical66e789_exact20_${STAMP}" \
  SESSION="lhs_t${TASK_ID}_historical66e789_${STAMP}" \
  JOB_NAME="lhs_t${TASK_ID}_historical66e789_${STAMP}" \
