...
# Copied byte-for-byte from the archived sync end-pose hold evaluator with
# SHA256 ef95604ca17c7900eac172d0e082a3738ca5b62e8468bf4f53c522590ff7dd2b,
# then adapted only at the current OpenHelix d9 callback boundary below.
...
def run_episode_sync_endpose_hold_d9_compat(
    *,
    task_id: int,
    env: Any,
    client: Any,
    ...
    """Adapt only the d9 callback shape around the frozen rollout body."""
    planner_task_id = int(planner.task_info.task_id)
    if planner_task_id != int(task_id):
        raise ValueError(f"task-id mismatch: callback={task_id} planner={planner_task_id}")

    stage_pct, stage_done, goal_success, replay, replay_wrist = run_episode_sync_endpose_hold(
        env=env,
        client=client,
        planner=planner,
        args=args,
    ...
    logger.info(
        "[D9_CALLBACK_COMPAT] task=%s stage_success=%s fail_on_extra_pour=%s "
        "extra_pour_monitor_steps=%s post_goal_steps=%s",
        planner_task_id,
        int(stage_success),
        int(bool(fail_on_extra_pour)),
    ...
    diagnostics = {
        ...
        "failure_reason": "archived_sync_endpose_hold_d9_callback_compat",
    }
    return stage_pct, stage_done, bool(goal_success), diagnostics, replay, replay_wrist


def main() -> None:
    os.environ["ASYNC_VLM"] = "0"
    base.FullVlm26MemoryPlanner._build_messages = _build_messages_runtime_progress
    base.run_episode_async_stateful = run_episode_sync_endpose_hold_d9_compat
    base.main()
