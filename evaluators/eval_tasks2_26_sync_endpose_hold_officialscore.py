...
base_eval_env = os.environ.get("TASKS2_26_BASE_EVAL_PY")
if not base_eval_env:
    raise RuntimeError("TASKS2_26_BASE_EVAL_PY must point to the pinned official checkout")
BASE_EVAL_PY = Path(base_eval_env)
...
official_scripts_env = os.environ.get("ROBOMEMARENA_OFFICIAL_SCRIPTS_DIR")
official_bddl_env = os.environ.get("ROBOMEMARENA_OFFICIAL_BDDL_DIR")
if not official_scripts_env or not official_bddl_env:
    raise RuntimeError(
        "ROBOMEMARENA_OFFICIAL_SCRIPTS_DIR and ROBOMEMARENA_OFFICIAL_BDDL_DIR "
        "must point to the pinned official checkout"
    )
OFFICIAL_SCRIPTS_DIR = Path(official_scripts_env)
OFFICIAL_BDDL_DIR = Path(official_bddl_env)
...
def run_episode_sync_endpose_hold(
    ...
    **_: Any,
    ...
    logger.info(
        ...
        "microwave_forward_require_prior_hold=%s "
    ...
    try:
        ...
        while t < args.max_steps + args.num_steps_wait:
            ...
            if oracle_stage_lock_until_done and (not hold_active) and current_subtask_prompt and latest_subtask:
                current_idx = order_index(current_subtask_prompt, labels)
                latest_idx = order_index(latest_subtask, labels)
                ...
                if (
                    ...
                    logger.info(
                        "[ORACLE_STAGE_LOCK_UNTIL_DONE] t=%s task=%s raw_subtask=%s "
            ...
            if require_hold_release_for_pick_forward and (not hold_active) and current_subtask_prompt and latest_subtask:
                current_idx = order_index(current_subtask_prompt, labels)
                latest_idx = order_index(latest_subtask, labels)
                if (
                    current_subtask_prompt.startswith("pick ")
                    and current_idx is not None
                    and latest_idx is not None
                    and latest_idx > current_idx
                    and latest_subtask != current_subtask_prompt
    ...
    diagnostics = {
        ...
        "failure_reason": "" if stage_success else (
            "open_eef_hold_missing" if official_stage_success_raw and not open_eef_contract_ok else "stage_incomplete"
        ),
