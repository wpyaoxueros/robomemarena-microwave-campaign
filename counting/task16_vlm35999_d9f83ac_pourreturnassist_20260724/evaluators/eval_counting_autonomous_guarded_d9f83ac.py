...
from autonomous_prompt_guard import AutonomousPromptGuard, PromptPollSchedule, Task8OraclePickAfterPlace
...
def _post_pour_hold_enabled() -> bool:
    ...
    if os.environ.get("HOLD_AFTER_REQUIRED_STAGES", "0").strip().lower() not in {"1", "true", "yes", "on"}:
        return False
    if _ACTIVE_PLANNER is None or _ACTIVE_PLANNER.task_info.task_id not in {6, 7, 10, 15, 16}:
        return False
    return _ACTIVE_PLANNER.prompt_guard.stage_count >= 3


def _stage_latch_hold_enabled() -> bool:
    if os.environ.get("STAGE_LATCH_AUTONOMOUS_HOLD", "0").strip().lower() not in {"1", "true", "yes", "on"}:
        return False
    if _ACTIVE_PLANNER is None or _ACTIVE_PLANNER.task_info.task_id not in {6, 7, 10, 15, 16}:
        return False
    return _ACTIVE_PLANNER.prompt_guard.stage_latch_pending


class PostPourHoldClient(_ORIGINAL_POLICY_CLIENT):
    """Hold counting-task actions after a completed stage without writing prompts."""
    ...
    def infer(self, element):
        if (_post_pour_hold_enabled() or _stage_latch_hold_enabled()) and self._last_output is not None:
            output = dict(self._last_output)
            output["actions"] = np.zeros_like(np.asarray(output["actions"]))
            if _stage_latch_hold_enabled() and _ACTIVE_PLANNER is not None:
                stage_count = _ACTIVE_PLANNER.prompt_guard.stage_count
                if getattr(_ACTIVE_PLANNER, "stage_latch_hold_logged_count", None) != stage_count:
                    _ACTIVE_PLANNER.stage_latch_hold_logged_count = stage_count
                    if _ACTIVE_PLANNER.logger is not None:
                        _ACTIVE_PLANNER.logger.info(
                            "[AUTONOMOUS_STAGE_LATCH_HOLD] task=%s stage_count=%s; waiting for VLM forward prompt",
                            _ACTIVE_PLANNER.task_info.task_id,
                            stage_count,
                        )
            elif _ACTIVE_PLANNER is not None and not getattr(_ACTIVE_PLANNER, "post_pour_hold_logged", False):
...
class GuardedPlanner(base.FullVlm26MemoryPlanner):
    ...
    def set_task_info(self, task_info) -> None:
        ...
        self.prompt_guard = AutonomousPromptGuard(
            ...
            stage_latch=os.environ.get("STAGE_LATCH_AUTONOMOUS_HOLD", "0").strip().lower()
            in {"1", "true", "yes", "on"},
            no_prompt_regression=os.environ.get("PROMPT_NO_REGRESSION", "0").strip().lower()
            in {"1", "true", "yes", "on"},
        )
        self.prompt_poll = PromptPollSchedule(
            interval=int(os.environ.get("VLM_INTERVAL", "5")),
        )
        self.task8_oracle_pick_after_place = Task8OraclePickAfterPlace(
            delay_steps=int(os.environ.get("ORACLE_TASK8_PICK_AFTER_PLACE_STEPS", "-1")),
        )
    ...
    def reset_episode(self, instruction: str | None = None, run_dir=None, logger=None):
        ...
        self.task8_oracle_pick_after_place.reset()
        self.post_pour_hold_logged = False
        self.stage_latch_hold_logged_count = None
        return result

    def infer_sync(self, step_idx: int, context_frames_np):
        if (
            not self.prompt_guard.stage_latch_pending
            and not self.prompt_poll.should_infer(step_idx)
            and self.prompt_guard.accepted_prompt
        ):
        ...
        was_latched = self.prompt_guard.stage_latch_pending
        ...
        if self.task8_oracle_pick_after_place.should_force(
            task_id=self.task_info.task_id,
            step_idx=step_idx,
            accepted_prompt=decision.prompt,
            stage_count=self.prompt_guard.stage_count,
        ):
            decision = self.prompt_guard.force_prompt("pick tomato sauce", raw_prompt=decision.raw_prompt)
            if self.logger is not None:
                self.logger.info(
                    "[ORACLE_TASK8_FORCE_PICK_AFTER_PLACE] t=%s task=8 raw=%s forced=%s delay_steps=%s stage_count=%s",
                    step_idx,
                    decision.raw_prompt,
                    decision.prompt,
                    self.task8_oracle_pick_after_place.delay_steps,
                    decision.stage_count,
                )
        if self.prompt_guard.stage_latch_pending:
            self.prompt_poll.force_next()
        elif was_latched and self.logger is not None:
            self.logger.info(
                "[AUTONOMOUS_STAGE_LATCH_RELEASE] t=%s task=%s raw=%s",
                step_idx,
                self.task_info.task_id,
                decision.raw_prompt,
            )
