...
def main(args: Args) -> None:
    if args.seed is not None:
        ...
        logging.info("Pinned host policy sampling seed: %s", args.seed)
    policy = create_policy(args)
    if args.seed is not None and not getattr(policy, "_is_pytorch_model", False):
        # OpenPI JAX policies keep their action-sampling PRNG on the policy
        # instance. Python/NumPy/Torch seeds do not affect this key.
        import jax

        policy._rng = jax.random.key(args.seed)
        logging.info("Pinned JAX policy sampling key: %s", args.seed)
