#!/usr/bin/env bash
# Run inside an existing two-GPU Slurm allocation. VLM remains the prompt source.
set -euo pipefail
...
export VLA_POLICY_SEED="${VLA_POLICY_SEED:-100}"
export MAX_STEPS="${MAX_STEPS:-2500}"
export REPLAN_STEPS=1
export POST_STAGE_STEPS=30
export VLM_INTERVAL="${VLM_INTERVAL:-25}"
export HOLD_AFTER_REQUIRED_STAGES=1
export STAGE_LATCH_AUTONOMOUS_HOLD=0
export PROMPT_NO_REGRESSION=1
export POUR_RETURN_ASSIST=0
export TASK10_POUR_RETURN_ASSIST=1
export TASK10_POUR_RETURN_ASSIST_TARGET_RADIUS=0.20
export TASK10_POUR_RETURN_ASSIST_ROTATION_MAGNITUDE=0.8
export TASK10_POUR_RETURN_ASSIST_MAX_STEPS=24
...
export EVALUATOR_FILE_OVERRIDE="${PACK_DIR}/evaluators/eval_counting_task10_pour_return_assist_d9f83ac.py"
export RUN_ID="${RUN_ID:-task10_vlm35999_pourreturnassist_${NUM_TRIALS}ep_$(date +%Y%m%d_%H%M%S)}"
