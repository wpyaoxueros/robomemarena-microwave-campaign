...
SOURCE_ROOT="${SOURCE_ROOT:?Set SOURCE_ROOT to a RoboMemArena checkout frozen at d9f83ac5182e25ad7f0a301a77a0b667f2392df1.}"
...
EVALUATOR_FILE="${EVALUATOR_FILE_OVERRIDE:-}"
...
VLA_CKPT="${VLA_CKPT:?Set VLA_CKPT to the original fullvlm-v2 35999 checkpoint.}"
VLA_CONFIG="${VLA_CONFIG:-pi05_libero_robomemarena_fullvlm_v2_noflip_dataset}"
OPENPI_ROOT="${OPENPI_ROOT:?Set OPENPI_ROOT to the compatible OpenPI checkout.}"
OPENPI_INFERENCE_ROOT="${OPENPI_INFERENCE_ROOT:?Set OPENPI_INFERENCE_ROOT to the compatible openpi_inference checkout.}"
...
POUR_TILT_TRACE_CSV="${POUR_TILT_TRACE_CSV:-${OUT_ROOT}/logs/pour_tilt_trace.csv}"

# The wrapper retains the remote evaluator/scorer and only implements opt-in guard mechanics.
if [[ -z "${EVALUATOR_FILE}" && ( "${PROMPT_NO_REGRESSION}" == "1" || "${HOLD_AFTER_REQUIRED_STAGES}" == "1" || "${STAGE_LATCH_AUTONOMOUS_HOLD}" == "1" ) ]]; then
  EVALUATOR_FILE="${COUNTING_GUARDED_EVALUATOR}"
fi
EVALUATOR_FILE="${EVALUATOR_FILE:-${OFFICIAL_EVALUATOR_FILE}}"
...
if [[ "${POUR_RETURN_ASSIST}" == "1" && "${TASK_ID}" != "16" ]]; then
  echo "[ERROR] POUR_RETURN_ASSIST is currently implemented only for Task16." >&2
  exit 2
fi
if [[ "${POUR_RETURN_ASSIST}" == "1" && "${REPLAN_STEPS}" != "1" ]]; then
  echo "[ERROR] Task16 pour-return assist requires REPLAN_STEPS=1 for per-step tilt feedback." >&2
  exit 2
fi
if [[ "${POUR_RETURN_ASSIST}" == "1" && "${TASK10_POUR_RETURN_ASSIST}" == "1" ]]; then
  echo "[ERROR] Enable at most one task-specific pour-return assist." >&2
  exit 2
fi
if [[ "${TASK10_POUR_RETURN_ASSIST}" == "1" && "${TASK_ID}" != "10" ]]; then
  echo "[ERROR] TASK10_POUR_RETURN_ASSIST is implemented only for Task10." >&2
  exit 2
fi
if [[ "${TASK10_POUR_RETURN_ASSIST}" == "1" && "${REPLAN_STEPS}" != "1" ]]; then
  echo "[ERROR] Task10 pour-return assist requires REPLAN_STEPS=1 for per-step tilt feedback." >&2
  exit 2
fi
...

{
  ...
  if [[ "${POUR_RETURN_ASSIST}" == "1" ]]; then
    echo "controller_assist=task16_rotation_return_only"
  ...
  elif [[ "${TASK10_POUR_RETURN_ASSIST}" == "1" ]]; then
    echo "controller_assist=task10_rotation_return_only"
    ...
    echo "controller_assist=off"
  fi
  echo "stage_prompt_override=off"
  echo "oracle_task8_pick_after_place_steps=${ORACLE_TASK8_PICK_AFTER_PLACE_STEPS}"
  if [[ "${ORACLE_TASK8_PICK_AFTER_PLACE_STEPS}" -ge 0 ]]; then
    echo "oracle_prompt_injection=task8_pick_after_place"
  else
  ...
  echo "pour_tilt_trace_csv=${POUR_TILT_TRACE_CSV}"
...
export VLM_INTERVAL HOLD_AFTER_REQUIRED_STAGES STAGE_LATCH_AUTONOMOUS_HOLD PROMPT_NO_REGRESSION POUR_RETURN_ASSIST
export POUR_RETURN_ASSIST_TARGET_RADIUS POUR_RETURN_ASSIST_ROTATION_MAGNITUDE POUR_RETURN_ASSIST_MAX_STEPS
export TASK10_POUR_RETURN_ASSIST TASK10_POUR_RETURN_ASSIST_TARGET_RADIUS
export TASK10_POUR_RETURN_ASSIST_ROTATION_MAGNITUDE TASK10_POUR_RETURN_ASSIST_MAX_STEPS
export ORACLE_TASK8_PICK_AFTER_PLACE_STEPS
export TASK6_TILT_TRACE_CSV="${POUR_TILT_TRACE_CSV}"
