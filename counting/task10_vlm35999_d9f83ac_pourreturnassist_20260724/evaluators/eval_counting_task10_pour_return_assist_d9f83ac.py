#!/usr/bin/env python3
"""Task10 VLM-prompted pour evaluator with an explicit return-to-upright assist.

This is intentionally a separate evaluator from the autonomous guarded path.
The VLM still supplies every prompt. Once Task10 has completed Lift, its own
accepted prompt is a wine-pour prompt, the wine bottle is already over the
white/yellow mug, and the official body tilt has crossed the pour threshold,
this wrapper only overrides rotational action channels to return the held
wine bottle upright. It
...
_TASK_ID = 10
_OBJECT_NAME = "wine_bottle_1"
_TARGET_KIND = "site"
_TARGET_NAME = "white_yellow_mug_1_default_site"
_MOVE_THRESHOLD = 0.15
_RETURN_THRESHOLD = 0.10
...
def _active_task10_pour_prompt() -> bool:
    ...
    return prompt in {
        "pour wine into mug 1st",
        "pour wine into mug 2nd",
    }
...
def _assist_enabled() -> bool:
    planner = guarded._ACTIVE_PLANNER
    return bool(
        _truthy(os.environ.get("TASK10_POUR_RETURN_ASSIST"))
...
def _task10_snapshot(env: Any) -> tuple[float, float, bool]:
    ...
    if tilt is None or source is None or target is None:
        raise RuntimeError(
            "Task10 pour-return assist cannot read wine-bottle tilt or white/yellow-mug target center."
        )
    radius = float(os.environ.get("TASK10_POUR_RETURN_ASSIST_TARGET_RADIUS", _DEFAULT_TARGET_RADIUS))
...
def _build_return_action(raw_action: Any, last_vla_rotation: Any) -> np.ndarray:
    ...
    if raw.size < 7 or rotation.size != 3:
        raise ValueError(
            f"Task10 pour-return assist expects a 7-D action and 3-D rotation, got {raw.shape}/{rotation.shape}."
        )
    rotation_norm = float(np.linalg.norm(rotation))
    if rotation_norm < 1e-6:
        raise ValueError("Task10 pour-return assist has no nonzero VLA rotation to reverse.")
    magnitude = float(os.environ.get("TASK10_POUR_RETURN_ASSIST_ROTATION_MAGNITUDE", "0.8"))
    action = raw.copy()
...
def _install_pour_return_env_patch() -> None:
    ...
    if not _truthy(os.environ.get("TASK10_POUR_RETURN_ASSIST")):
        return

    env_cls = base.ec._get_env_class()
    if getattr(env_cls, "_task10_pour_return_assist_installed", False):
        return
    ...
    def patched_reset(self, *args: Any, **kwargs: Any):
        ...
        self._task10_pour_return_assist_state = _new_assist_state()
        return result
    ...
    def patched_step(self, action: Any):
        ...
        state = getattr(self, "_task10_pour_return_assist_state", None)
        if state is None:
            state = _new_assist_state()
            self._task10_pour_return_assist_state = state
        ...
        if state["last_stage_count"] != stage_count:
            ...
            _log(
                "[TASK10_POUR_RETURN_ASSIST_STAGE_RESET] task=10 stage_count=%s prompt=%s",
                stage_count,
                planner.prompt_guard.accepted_prompt,
            )

        # Pour stages start after Lift and stop after the required two pours.
        if stage_count < 1 or stage_count >= 3 or not _active_task10_pour_prompt():
            return original_step(self, action)

        raw_action = np.asarray(action, dtype=np.float32).reshape(-1)
        if raw_action.size < 7:
            raise ValueError(f"Task10 pour-return assist received malformed action shape {raw_action.shape}.")

        tilt_before, distance_before, in_target_before = _task10_snapshot(self)
        if state["baseline_tilt"] is None:
            state["baseline_tilt"] = float(tilt_before)
            _log(
                "[TASK10_POUR_RETURN_ASSIST_BASELINE] task=10 stage_count=%s tilt=%.5f target_xy=%.5f prompt=%s",
                stage_count,
                tilt_before,
                distance_before,
                planner.prompt_guard.accepted_prompt,
        ...
        if state["return_active"]:
            ...
            if state["return_steps"] in {1, 5, 10, 20}:
                _log(
                    "[TASK10_POUR_RETURN_ASSIST_STEP] task=10 cycle=%s step=%s tilt=%.5f target_xy=%.5f raw_rot=%s applied_rot=%s",
        ...
        tilt_after, distance_after, in_target_after = _task10_snapshot(self)
        ...
        if not state["return_active"]:
            if (
                ...
                state["return_steps"] = 0
                _log(
                    "[TASK10_POUR_RETURN_ASSIST_ARM] task=10 cycle=%s baseline=%.5f tilt=%.5f deviation=%.5f target_xy=%.5f rotation=%s",
                    state["return_cycles"] + 1,
                    baseline,
                    tilt_after,
                    deviation_after,
        ...
        max_steps = int(os.environ.get("TASK10_POUR_RETURN_ASSIST_MAX_STEPS", "24"))
        if deviation_after <= _RETURN_THRESHOLD:
            ...
            state["last_vla_rotation"] = None
            _log(
                "[TASK10_POUR_RETURN_ASSIST_DONE] task=10 cycle=%s tilt=%.5f deviation=%.5f target_xy=%.5f",
                state["return_cycles"],
                tilt_after,
                deviation_after,
                distance_after,
            ...
            state["last_vla_rotation"] = None
            _log(
                "[TASK10_POUR_RETURN_ASSIST_TIMEOUT] task=10 tilt=%.5f deviation=%.5f target_xy=%.5f max_steps=%s",
                tilt_after,
                deviation_after,
                distance_after,
                max_steps,
    ...
    env_cls._task10_pour_return_assist_installed = True
...
if __name__ == "__main__":
    if not _truthy(os.environ.get("TASK10_POUR_RETURN_ASSIST")):
        raise RuntimeError("Set TASK10_POUR_RETURN_ASSIST=1 to run this controller-assisted evaluator.")
